...
/// @desc Draws the border of a circle at a given thickness. If the thickness meets or exceeds the radius, the circle is filled entirely.
...
/// @param {Real} thickness The thickness of the border in pixels.
function draw_circle_border(x, y, radius, thickness)
{
	var _inner = radius - thickness;
	if ( _inner <= 0 )
	{
...
/// @desc Draws a line from one point to another with a filled arrowhead at the endpoint.
/// @param {Real} x1 The x position of the start point.
/// @param {Real} y1 The y position of the start point.
/// @param {Real} x2 The x position of the end point.
/// @param {Real} y2 The y position of the end point.
/// @param {Real} head_size The size of the arrowhead in pixels.
/// @param {Real} [thickness] The thickness of the line in pixels. Defaults to 1.
function draw_arrow(x1, y1, x2, y2, head_size, thickness = 1)
{
	var _angle = point_direction(x1, y1, x2, y2);
	var _left  = _angle + 145;
	var _right = _angle - 145;
	
	var _lx = x2 + lengthdir_x(head_size, _left);
	var _ly = y2 + lengthdir_y(head_size, _left);
	var _rx = x2 + lengthdir_x(head_size, _right);
	var _ry = y2 + lengthdir_y(head_size, _right);
	
	var _base_x = (_lx + _rx) / 2;
	var _base_y = (_ly + _ry) / 2;
	
	draw_line_width(x1, y1, _base_x, _base_y, thickness);
	draw_triangle(_lx, _ly, x2, y2, _rx, _ry, false);
}